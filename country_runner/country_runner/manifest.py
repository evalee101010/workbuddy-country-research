...
def _render_context(template: str, config: dict, run_id: str) -> str:
    ...
    replacements = {
        ...
        "{{run_id}}": run_id,
        "{{window_start}}": str(research["window_start"]),
        "{{window_end}}": str(research["window_end"]),
...
def initialize_run(
    ...
    (run_dir / "01-country-context.md").write_text(
        _render_context(context_template, config, run_id), encoding="utf-8"
    )
    ...
    manifest = {
        ...
        "reviewer": "",
        "research_window": {
            "start": str(config["research"]["window_start"]),
            "end": str(config["research"]["window_end"]),
        },
